...
- **Go 1.26**, `net/http` + [chi/v5](https://github.com/go-chi/chi) роутер
- **PostgreSQL 16**, драйвер [`pgx/v5`](https://github.com/jackc/pgx) (без ORM)
- Миграции — [`golang-migrate`](https://github.com/golang-migrate/migrate), запускаются автоматически при старте
- Конфиг — `config.yaml` + переменные окружения через [`cleanenv`](https://github.com/ilyakaznacheev/cleanenv)
- Логи — стандартный `log/slog`, JSON-формат
- Swagger — [`swaggo/swag`](https://github.com/swaggo/swag) генерирует спецификацию из аннотаций
- Docker compose поднимает postgres + сервис
...
- **Слои `handler → service → repository`.** Хендлер парсит HTTP и валидирует формат, сервис — бизнес-правила, репозиторий — SQL. Сервис не знает про HTTP, репозиторий не знает про сервис.
- **Кастомный тип `MonthYear`** инкапсулирует формат `MM-YYYY` в одной точке. JSON-(де)сериализация и драйвер БД (`Value`/`Scan`) реализованы как методы типа — выше по стеку с ним работают как с обычным значением.
- **Без ORM.** Все запросы написаны руками. Это даёт прозрачность и контроль над SQL — особенно над агрегирующим расчётом `total`, который выполняется на стороне БД.
- **Миграции применяются на старте сервиса.** Удобно для локальной разработки и docker compose; в проде это обычно отдельный шаг CI/CD, но для тестового — достаточно.
- **Структурированные логи.** Каждый запрос пишет одну строку (метод, путь, статус, длительность, request_id). Внутренние ошибки логируются на уровне ERROR с контекстом, в ответ при этом уходит обезличенное `"internal error"`.
- **Graceful shutdown** по `SIGINT`/`SIGTERM`: даём текущим запросам завершиться, потом закрываем пул.
...
Приоритет источников (выше = главнее):

1. переменные окружения
2. `config.yaml` (путь — `CONFIG_PATH`, дефолт `./config.yaml`)
3. дефолты из тегов структуры
